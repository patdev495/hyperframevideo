...
- [x] `CandidateSelector.select(candidates)` prints a numbered menu to stdout.
- [x] Menu shows index, title, source name, publish date, and summary for each candidate.
- [x] Valid index input returns the corresponding `NewsCandidate`.
- [x] Invalid input (non-integer, out-of-range) re-prompts without crashing.
- [x] Input of `0` or `q` signals re-run (returns `None` or a typed sentinel).
- [x] Tests inject fixture candidates and simulate stdin; cover valid selection, invalid input loop, and re-run signal - no real terminal required.
